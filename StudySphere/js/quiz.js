...
function submitQuiz() {
  ...
  const result = { attemptId: `attempt-${Date.now()}`, examId: exam.id, examTitle: exam.title, submittedAt: new Date().toISOString(), score: Number(((correctCount / questions.length) * 10).toFixed(2)), totalQuestions: questions.length, correctCount, wrongCount: answers.filter((answer, index) => answer !== null && answer !== questions[index].correctAnswer).length, unansweredCount: answers.filter((answer) => answer === null).length, answers };
